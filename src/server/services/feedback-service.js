...
async function createFeedback(payload, req) {
    ...
    if (!type || !ALLOWED_TYPES.includes(type)) {
        throw new AppError({ code: 'BAD_REQUEST', statusCode: 400, message: '反馈类型无效' });
    }
    const pri = ALLOWED_PRIORITIES.includes(priority) ? priority : 'medium';
    if (!description || typeof description !== 'string' || description.trim().length === 0) {
        throw new AppError({ code: 'BAD_REQUEST', statusCode: 400, message: '请填写详细描述' });
    }
