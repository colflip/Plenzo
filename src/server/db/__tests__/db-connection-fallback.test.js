...
describe('database connection fallback', () => {
  ...
  test('interactive transactions fail safely when the transaction client is unavailable', async () => {
    ...
    await expect(db.runInTransaction(workFn)).rejects.toMatchObject({
      code: 'DB_UNAVAILABLE',
      message: '数据库暂时不可用，无法安全执行事务'
    });
